...
int main(){
    ...
    //문자열(1차 포인터)을 통합하는게 아닌 변환하여 값들을 통합(일차 포인터만으로 가능)
    int* numberbox = (int*)calloc(128, sizeof(int));   
    ...
    do
    {
        ...
        {   
            ...
            {
                sum += (int)((int)text[i] - 48)*pow(10.0, length - i -1);     //문자 '123' -> 100 + 20 + 3 = 123
            }
        }
    ...
    {
        printf("%d ", numberbox[i]);   // 포인터 역참조값 형변환 하여 나열
    }
    ...
    while(low <= high)                            // high가 더 클 때                     
    {
        ...
        if(key == numberbox[middle]) break;  //중간값이 키값일 때

        if (key > numberbox[middle])         //키값이 중간값보다 클 때
        {
            low = middle + 1;
        }
        else                                      //키값이 중간값보다 작을 때
